...
static FailureOr<double> runJIT() {
  // Keep the high-level MLIR intact and lower a clone for execution.
  OwningOpRef<ModuleOp> JITModule = TheModule->clone();
  ...
  if (failed(LoweringPM.run(*JITModule))) {
    LogError("Could not lower module to the LLVM dialect.");
    return failure();
  }
  ...
  auto MaybeEngine = ExecutionEngine::create(*JITModule, EngineOptions);
  if (!MaybeEngine) {
    llvm::errs() << "Error: could not create execution engine: "
  ...
  Engine->registerSymbols([](llvm::orc::MangleAndInterner Interner) {
    ...
    Symbols[Interner("putchard")] = {
        llvm::orc::ExecutorAddr::fromPtr(&putchard),
        llvm::JITSymbolFlags::Exported};
    Symbols[Interner("printd")] = {llvm::orc::ExecutorAddr::fromPtr(&printd),
                                   llvm::JITSymbolFlags::Exported};
    return Symbols;
  });

  double Result = 0.0;
  SmallVector<void *> Args{&Result};
  if (llvm::Error Error = Engine->invokePacked("__anon_expr", Args)) {
...
static void HandleTopLevelExpression() {
  ...
  if (auto FnAST = ParseTopLevelExpr()) {
    if (auto FnIR = FnAST->codegen()) {
      FailureOr<double> Result = runJIT();
      if (succeeded(Result))
